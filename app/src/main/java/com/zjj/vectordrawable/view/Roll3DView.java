...
public class Roll3DView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (rollMode){
            case Roll2D:
                drawRollWhile3D(canvas,true);
                break;
            case whole3D:
                drawRollWhile3D(canvas,false);
                break;
            case SepartConbine:
     ...
     */
    private void drawRollWhile3D(Canvas canvas, boolean draw2D) {
        ...
        if(direction == 1){
            ...
            matrix.preTranslate(-viewWidth / 2,0);//确定旋转轴
            matrix.postTranslate(viewWidth / 2, axisY);//下面的view绕着自己上边转，转完之后显示要往下平移
            ...
            matrix.preTranslate(-viewWidth/2,-viewHeight);
            matrix.postTranslate(viewWidth / 2,axisY);//上面的View绕着自己下边转，转完之后显示要往上平移
    ...
    private void drawRollInTurn(Canvas canvas) {
        for (int i = 0; i < partNumber; i++) {
            ...
            if(direction == 1){
                ...
                camera.getMatrix(matrix);
                camera.restore();
                //先将旋转的边移动移动到坐标轴原点（0,0）
                matrix.preTranslate(0, -currBitmap.getHeight() / 2);
                //让后再平移回来，让后在旋转距离tAxisX,currBitmap.getHeight() / 2 + i * averageHeight是原来高度
